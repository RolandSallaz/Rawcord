...
interface ContextMenu { peer: ParticipantInfo; x: number; y: number }
...
const IconMic = () => <svg viewBox="0 0 24 24" fill="currentColor"><path d="M12 14c1.66 0 2.99-1.34 2.99-3L15 5c0-1.66-1.34-3-3-3S9 3.34 9 5v6c0 1.66 1.34 3 3 3zm5.3-3c0 3-2.54 5.1-5.3 5.1S6.7 14 6.7 11H5c0 3.41 2.72 6.23 6 6.72V21h2v-3.28c3.28-.48 6-3.3 6-6.72h-1.7z"/></svg>
const IconMicOff = () => <svg viewBox="0 0 24 24" fill="currentColor"><path d="M19 11h-1.7c0 .74-.16 1.43-.43 2.05l1.23 1.23c.56-.98.9-2.09.9-3.28zm-4.02.17c0-.06.02-.11.02-.17V5c0-1.66-1.34-3-3-3S9 3.34 9 5v.18l5.98 5.99zM4.27 3L3 4.27l6.01 6.01V11c0 1.66 1.33 3 2.99 3 .22 0 .44-.03.65-.08l1.66 1.66c-.71.33-1.5.52-2.31.52-2.76 0-5.3-2.1-5.3-5.1H5c0 3.41 2.72 6.23 6 6.72V21h2v-3.28c.91-.13 1.77-.45 2.54-.9L19.73 21 21 19.73 4.27 3z"/></svg>
const IconHeadphones = () => <svg viewBox="0 0 24 24" fill="currentColor"><path d="M3 9v6h4l5 5V4L7 9H3zm13.5 3c0-1.77-1.02-3.29-2.5-4.03v8.05c1.48-.73 2.5-2.25 2.5-4.02zM14 3.23v2.06c2.89.86 5 3.54 5 6.71s-2.11 5.85-5 6.71v2.06c4.01-.91 7-4.49 7-8.77s-2.99-7.86-7-8.77z"/></svg>
const IconHeadphonesOff = () => <svg viewBox="0 0 24 24" fill="currentColor"><path d="M16.5 12c0-1.77-1.02-3.29-2.5-4.03v2.21l2.45 2.45c.03-.2.05-.41.05-.63zm2.5 0c0 .94-.2 1.82-.54 2.64l1.51 1.51C20.63 14.91 21 13.5 21 12c0-4.28-2.99-7.86-7-8.77v2.06c2.89.86 5 3.54 5 6.71zM4.27 3L3 4.27 7.73 9H3v6h4l5 5v-6.73l4.25 4.25c-.67.52-1.42.93-2.25 1.18v2.06c1.38-.31 2.63-.95 3.69-1.81L19.73 21 21 19.73l-9-9L4.27 3zM12 4L9.91 6.09 12 8.18V4z"/></svg>
const IconScreen = () => <svg viewBox="0 0 24 24" fill="currentColor"><path d="M20 3H4c-1.1 0-2 .9-2 2v11c0 1.1.9 2 2 2h3l-1 1v2h12v-2l-1-1h3c1.1 0 2-.9 2-2V5c0-1.1-.9-2-2-2zm0 13H4V5h16v11z"/></svg>
...
export default function ChannelPage() {
  ...
  // Remote streams: peerId → HTMLVideoElement (managed by WsAudioClient)
  const [remoteStreams, setRemoteStreams] = useState<Map<string, HTMLVideoElement>>(new Map())
  ...
  // Context menu (right-click on participant)
  ...
  // Clear watching if stream ended
  useEffect(() => {
    if (watchingPeerId && !remoteStreams.has(watchingPeerId)) setWatchingPeerId(null)
  }, [remoteStreams, watchingPeerId])
  ...
  function scheduleReconnect() {
    ...
    setReconnectCountdown(remaining); setAppState('reconnecting')
  ...
  const connectToChannel = useCallback(async (wsUrl: string, owner: boolean) => {
    ...
    audio.onLocalSpeaking = (s) => setIsSpeaking(s)

    audio.onStreamStarted = (peerId, el) => {
      // Mount video element into our container
      videoContainerRef.current?.appendChild(el)
      setRemoteStreams(prev => new Map(prev).set(peerId, el))
    }
    audio.onStreamStopped = (peerId) => {
      const el = audioRef.current?.getVideoElement(peerId)
      el?.remove()
      setRemoteStreams(prev => { const m = new Map(prev); m.delete(peerId); return m })
    }

    signaling.on('onWelcome', async ({ participants, streamers }) => {
      setPeers(participants)
      audio.initStreamers(streamers)
    ...
    signaling.on('onStreamStarted', (_id, _mimeType) => {
      // Handled by WsAudioClient.onStreamStarted callback above
    })
    signaling.on('onStreamStopped', (_id) => {
      // Handled by WsAudioClient.onStreamStopped callback above
    })
    ...
    setIsOwner(owner); setAppState('connected')
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [settings, profile])

  function handleDisconnect() {
    intentionalDisconnectRef.current = true; cancelReconnect()
    if (isSharing) { audioRef.current?.stopScreenShare(); setIsSharing(false) }
    setRemoteStreams(new Map()); setWatchingPeerId(null); setContextMenu(null)
    cleanup(); setPeers([]); setMicMuted(false); setDeafened(false)
    prevMicMutedRef.current = false; setServerUrl('')
    if (isOwner) ipcRenderer.invoke('server:stop').catch(() => {})
    setIsOwner(false); lastConnectionRef.current = null; setAppState('idle')
  }
  ...
  function toggleDeafen() {
    ...
    if (next) { prevMicMutedRef.current = micMuted; setMicMuted(true); audioRef.current?.setMicMuted(true) }
    else { setMicMuted(prevMicMutedRef.current); audioRef.current?.setMicMuted(prevMicMutedRef.current) }
  ...
  async function handleStartServer() {
    ...
    if (isNaN(port) || port < 1024 || port > 65535) { setErrorMsg('Укажите порт от 1024 до 65535'); setAppState('error'); return }
    try {
      ...
      setServerUrl(publicUrl); lastConnectionRef.current = { url: publicUrl, isOwner: true }
      await connectToChannel(url, true)
    } catch (e) {
      const msg = e instanceof Error ? e.message : ''
      setErrorMsg(msg.includes('EADDRINUSE') ? `Порт ${serverPort} уже занят` : (msg || 'Не удалось запустить сервер'))
      setAppState('error')
    }
  }
  ...
  async function handleStartScreenShare(stream: MediaStream, _sourceId: string, _withAudio: boolean) {
    ...
    try {
      ...
      console.error('[ChannelPage] startScreenShare failed:', e)
    }
  }

  function handleStopScreenShare() {
    audioRef.current?.stopScreenShare()
    setIsSharing(false)
  }
  ...
  function handleSendChat() {
    ...
    setChatMessages(prev => [...prev, {
      id: `self-${Date.now()}`, from: 'self', text,
      nickname: profile.nickname, avatar: profile.avatar || undefined, timestamp: Date.now(),
    }])
    setChatInput('')
  }
  ...
  const watchingEl = watchingPeerId ? remoteStreams.get(watchingPeerId) : null

  return (
    <div className="cp-root" onClick={() => setContextMenu(null)}>
      {settingsOpen && (
        <SettingsModal
          settings={settings} profile={profile}
          onChange={s => { setSettings(s); saveSettings(s); audioRef.current?.setOutputDevice(s.outputDeviceId) }}
          onProfileChange={p => { saveProfile(p); setProfile(p); signalingRef.current?.announce(p.nickname, p.avatar || undefined) }}
      ...
      {/* Hidden video container — WsAudioClient appends <video> elements here */}
      <div ref={videoContainerRef} style={{ display: 'none' }} />

      {/* Full-screen stream viewer */}
      {appState === 'connected' && watchingPeerId && watchingEl && (
        <div className="ss-fullscreen-overlay">
          <div className="ss-fullscreen-header">
            <span className="ss-fullscreen-title">
              {peers.find(p => p.id === watchingPeerId)?.nickname ?? watchingPeerId.slice(0, 8)} транслирует
            </span>
            <button className="ss-fullscreen-close" onClick={() => setWatchingPeerId(null)}>✕</button>
          </div>
          <div
            className="ss-fullscreen-video"
            ref={(el) => {
              if (el && watchingEl && !el.contains(watchingEl)) {
                el.innerHTML = ''
                el.appendChild(watchingEl)
      ...
      {contextMenu && (
        ...
        >
          {remoteStreams.has(contextMenu.peer.id) && (
            <button
              className="cp-context-item"
              onClick={() => { setWatchingPeerId(contextMenu.peer.id); setContextMenu(null) }}
            >
              <IconScreen /> Присоединиться к трансляции
            </button>
          )}
          <div className="cp-context-section">Громкость</div>
          <div className="cp-context-vol-row">
            <input
              type="range" min={0} max={200} step={1}
              value={peerVolumes.get(contextMenu.peer.id) ?? 100}
              onClick={e => e.stopPropagation()}
              onChange={e => {
                const v = parseInt(e.target.value)
                setPeerVolumes(prev => new Map(prev).set(contextMenu.peer.id, v))
                audioRef.current?.setPeerVolume(contextMenu.peer.id, v)
              }}
            />
            <span>{peerVolumes.get(contextMenu.peer.id) ?? 100}%</span>
          </div>
        </div>
      )}
            ...
            <div className="cp-home-actions">
              <div className="cp-action-card create-server-card">
                ...
                <div className="connect-input-row">
                  ...
                  <input className="server-url-input port-input" type="number" min="1024" max="65535" placeholder="3001" value={serverPort} onChange={e => setServerPort(e.target.value)} onKeyDown={e => { if (e.key === 'Enter') handleStartServer() }} />
              ...
              <div className="cp-action-card connect-card">
                ...
                <div className="connect-input-row">
                  <input className="server-url-input" type="text" placeholder="192.168.1.100:3001" value={serverInput} onChange={e => setServerInput(e.target.value)} onKeyDown={e => { if (e.key === 'Enter') handleConnectToServer() }} />
                  <button className="connect-submit-btn" disabled={!serverInput.trim()} onClick={() => handleConnectToServer()}>Подкл.</button>
                </div>
                  ...
                  <div className="cp-recent-servers">
                    ...
                    {recentServers.map(s => (
                      <div key={s.url} className="cp-recent-entry">
                        ...
                        <button className="cp-recent-remove" title="Удалить" onClick={() => { removeRecentServer(s.url); setRecentServers(loadRecentServers()) }}>×</button>
                      </div>
                    ))}
                  </div>
                )}
      ...
      {appState === 'connecting' && (
        <div className="cp-center">
          <div className="voice-icon pulse"><svg viewBox="0 0 24 24" fill="currentColor"><path d="M12 1c-4.97 0-9 4.03-9 9v7c0 1.66 1.34 3 3 3h3v-8H5v-2c0-3.87 3.13-7 7-7s7 3.13 7 7v2h-4v8h3c1.66 0 3-1.34 3-3v-7c0-4.97-4.03-9-9-9z"/></svg></div>
      ...
      {appState === 'reconnecting' && (
        <div className="cp-center">
          <div className="voice-icon pulse"><svg viewBox="0 0 24 24" fill="currentColor"><path d="M17.65 6.35C16.2 4.9 14.21 4 12 4c-4.42 0-7.99 3.58-7.99 8s3.57 8 7.99 8c3.73 0 6.84-2.55 7.73-6h-2.08c-.82 2.33-3.04 4-5.65 4-3.31 0-6-2.69-6-6s2.69-6 6-6c1.66 0 3.14.69 4.22 1.78L13 11h7V4l-2.35 2.35z"/></svg></div>
          ...
          <button className="connect-btn" style={{ marginTop: 16 }} onClick={() => { cancelReconnect(); intentionalDisconnectRef.current = true; setAppState('idle'); setServerUrl(''); lastConnectionRef.current = null }}>Отмена</button>
        </div>
      )}
      ...
      {appState === 'error' && (
        <div className="cp-center">
          <div className="voice-icon" style={{ color: 'var(--red)' }}><svg viewBox="0 0 24 24" fill="currentColor"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm1 15h-2v-2h2v2zm0-4h-2V7h2v6z"/></svg></div>
          ...
          <div className="cp-connected-body">
            ...
            <div className="cp-participants">
              ...
              {peers.map(peer => (
                <div
                  ...
                  onContextMenu={(e) => {
                    e.preventDefault()
                    setContextMenu({ peer, x: e.clientX, y: e.clientY })
                  }}
                >
                  ...
                  <div className="cp-participant-icons">
                    {remoteStreams.has(peer.id) && (
                      <svg className="vm-sharing-icon" viewBox="0 0 24 24" fill="currentColor" title="Транслирует экран"><path d="M20 3H4c-1.1 0-2 .9-2 2v11c0 1.1.9 2 2 2h3l-1 1v2h12v-2l-1-1h3c1.1 0 2-.9 2-2V5c0-1.1-.9-2-2-2zm0 13H4V5h16v11z"/></svg>
                    )}
            ...
            <div className="cp-main">
              {hasRemoteStream ? (
                <div className="cp-previews">
                  {[...remoteStreams.entries()].map(([peerId, el]) => (
                    <div
                      key={peerId}
                      className="ss-preview-wrap"
                      onClick={() => setWatchingPeerId(peerId)}
                    ...
                    >
                      ...
                      <div className="ss-preview-label">
                        ...
                        <span className="ss-preview-hint"> · нажмите для просмотра</span>
                      </div>
                    </div>
                  ))}
                </div>
                ...
                <div className="cp-voice-active">
                  <div className="cp-voice-waves">
                    <div className="cp-wave" /><div className="cp-wave" /><div className="cp-wave" />
                  </div>
                  <p className="cp-voice-active-label">Голосовой чат активен</p>
                  <p className="cp-voice-active-sub">{peers.length === 0 ? 'Ожидание участников…' : `${peers.length + 1} участн. в канале`}</p>
                </div>
              )}
            </div>
            ...
            <div className="cp-chat">
              ...
              <div className="cp-chat-input-bar">
                <input ref={chatInputRef} className="cp-chat-input" type="text" placeholder="Написать сообщение…" value={chatInput} onChange={e => setChatInput(e.target.value)} onKeyDown={e => { if (e.key === 'Enter') handleSendChat() }} />
          ...
          <div className="cp-bottom-bar">
            <div className="cp-bar-user">
              ...
              <div className="cp-bar-user-info">
                ...
                <span className="cp-bar-status">{deafened ? 'Заглушён' : micMuted ? 'Микрофон выкл.' : isSpeaking ? 'Говорит…' : 'В канале'}</span>
              </div>
            </div>

            <div className="cp-bar-controls">
              <button className={`cp-bar-btn${micMuted ? ' active-red' : ''}`} title={micMuted ? 'Включить микрофон' : 'Выключить микрофон'} onClick={toggleMic}>
                {micMuted ? <IconMicOff /> : <IconMic />}
                <span className="cp-bar-btn-label">{micMuted ? 'Вкл. mic' : 'Выкл. mic'}</span>
              </button>
              <button className={`cp-bar-btn${deafened ? ' active-red' : ''}`} title={deafened ? 'Включить звук' : 'Заглушить всех'} onClick={toggleDeafen}>
              ...
              {!isSharing
                ? <button className="cp-bar-btn" title="Трансляция экрана" onClick={() => setScreenShareOpen(true)}>
                    <IconScreen /><span className="cp-bar-btn-label">Экран</span>
                  </button>
                : <button className="cp-bar-btn active-green" title="Остановить трансляцию" onClick={handleStopScreenShare}>
                    <IconScreen /><span className="cp-bar-btn-label">Стоп</span>
                  </button>
              }
            </div>
            ...
            <div className="cp-bar-right">
              {serverUrl && isOwner && (
                <button className={`cp-copy-url-btn${copiedUrl ? ' copied' : ''}`} title="Скопировать адрес" onClick={copyServerUrl}>
                  {copiedUrl
